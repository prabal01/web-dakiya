...
exports.createProject = async (req, res) => {
  ...
  const user = await prisma.user.findFirst({where:{email:email}})
   ...
   const project = await prisma.project.create({
    ...
    res.status(200).send({publicVAPID:project.publicVAPID,name:project.name,createdAt:project.createdAt})
  // }
...
const {userId, email} = req.userInfo
  const projects = prisma.project.findMany({where:{userId}})

  console.log("🚀 ~ file: project-controller.js:75 ~ exports.getProject= ~ projects:", projects)
res.sendStatus(200)
}
