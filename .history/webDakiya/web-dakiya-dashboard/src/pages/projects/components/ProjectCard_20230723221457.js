...
export default function ProjectCard({ title,createdAt, vapidKey, icon, color = 'primary', sx, ...other }) {
  ...
  return (
    <Card
      ...
      {...other}
    >
      <div style={{display:'flex', justifyContent:"space-between", padding:'0 10px', marginTop:"-15px", marginBottom:"15px"}}>
        <div style={{display:'flex', alignItems:"center"}}>
          <Typography>Created At: {formatDate(createdAt)}</Typography>
        </div>
        <div >
          <IconButton>
