...
import {  CircularProgress, Grid, Container,Typography } from '@mui/material';
...
import styles from "../../globalStyles/customAnimations.css"

export default function Projects() {
  const [creatingProject, setCreatingProject] = useState(false);

  const [isProjectLoading, setIsProjectLoading] = useState(false);
  const {  _setSelectedProject, _projects, _setProjects } = useProjectStore((state) => ({
    // get
...
useEffect(() => {
  ...
  return <>
    ...
    { _projects.length > 0 &&
        ...
        _projects.map(project => <Grid item xs={12} md={4}><ProjectCard setSelectedProject={()=>_setSelectedProject(project)} deleteProjectHandler={()=>deleteProjectHandler(project.id)} createdAt={project.createdAt} vapidKey={project.publicVAPID} icon="mdi:push-notification" sx={{ margin: '10px' }} title={project.name} /></Grid>)}
